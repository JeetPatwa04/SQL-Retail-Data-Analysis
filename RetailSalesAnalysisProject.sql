...
SELECT 
	category,
	SUM(total_sale) as net_sale, -- saved as var net_sale
...
(
	SELECT *, 
		CASE
			...
			WHEN EXTRACT(HOUR FROM sale_time) BETWEEN 12 AND 17 THEN 'Afternoon' -- between ensures 12 and 17 are included 
			ELSE 'Evening'
		END as shift
	FROM retail_sales
)
SELECT 
	shift,
	COUNT(*) as total_orders --* somehow works as transactions_id
FROM hourly_sale
GROUP BY shift
